...
const Product = ({small, subTitle, price}) => {
  return (
    <Link to={'/details'} className="product p-12 ease-out border-2 border-white hover:border-black border-solid  text-center">
      <div className={`${small ? 'h-56' : 'h-100'} flex relative`}>
        <img className={`m-auto ${small ? 'h-56' : ''}`} src={dummyProduct} alt="product" />
        {small && <div className="w-16 flex h-7 absolute right-0 top-0 price-tag opacity-0" style={{ background: `url(${label}) no-repeat` }}>
      ...
      <p className="mt-5 text-xl w-full text-center">{subTitle}</p>
